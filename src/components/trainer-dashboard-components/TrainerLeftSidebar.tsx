...
export default function TrainerLeftSidebar({ hideOnMobile = false }: { hideOnMobile?: boolean } = {}) {
  ...
  const updateState = (data: any) => {
    setProfile({
      ...
      completion: (() => {
        let score = 20;
        if (data.bio && data.bio.length > 0) score += 40;
        if (data.expertise && data.expertise.length > 0) score += 40;
  ...
  return (
    <aside className={`lg:col-span-3 h-auto lg:h-full overflow-visible lg:overflow-y-auto scrollbar-none space-y-6 pb-6 ${hideOnMobile ? "hidden lg:block" : ""}`}>
      ...
      {/* Quick Links */}
      <div className="bg-white border border-slate-200 rounded-3xl p-4 shadow-xs mt-6">
        <Link 
          href="/trainer/settings?tab=payouts" 
