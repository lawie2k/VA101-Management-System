...
type VATrainingPaymentModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
  courseId: string;
...
export default function VATrainingPaymentModal({ isOpen, onClose, onSuccess, courseId, price }: VATrainingPaymentModalProps) {
  ...
  const handlePaymentClick = async (gateway: string) => {
    try {
      ...
      const res = await fetch("/api/va/training/buy", {
        method: "POST",
