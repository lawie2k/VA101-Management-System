...
export default function DashboardMainFeed({
  ...
  const activeStep = getStepNumber(mockApplication.status);
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-xs">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] font-bold text-slate-400">
            {activeStep > 0 ? `Applied ${mockApplication.appliedDate}` : "Inactive"}
          </span>
        </div>
        <div className="p-4 bg-slate-50 border border-slate-150 rounded-2xl">
          ...
          {activeStep > 0 ? (
            <div className="mt-5 grid grid-cols-4 gap-2 relative">
              ...
              {/* Screening Step */}
              <div className="text-center relative">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center mx-auto mb-1 transition-all ${activeStep >= 2 ? "bg-[#E84E29] text-white" : "bg-slate-200 text-slate-500 border border-slate-300"}`}>
                  {activeStep >= 2 ? <IconCheckCircle className="w-3.5 h-3.5" /> : <span className="text-[9px] font-bold">2</span>}
                </div>
                <span className={`text-[9px] font-bold ${activeStep >= 2 ? "text-slate-800" : "text-slate-400"}`}>Screening</span>
              </div>

              {/* Interview Step */}
              <div className="text-center relative">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center mx-auto mb-1 transition-all ${activeStep >= 3 ? "bg-[#E84E29] text-white" : "bg-slate-200 text-slate-500 border border-slate-300"}`}>
                  {activeStep >= 3 ? <IconCheckCircle className="w-3.5 h-3.5" /> : <span className="text-[9px] font-bold">3</span>}
                </div>
                <span className={`text-[9px] font-bold ${activeStep >= 3 ? "text-slate-800" : "text-slate-400"}`}>Interview</span>
              </div>
            ...
            <p className="text-[11px] text-slate-400 font-semibold mt-3 italic">
              You haven't submitted any job pitches yet. Open the Jobs catalog to get hired!
            </p>
          )}
        </div>
      </div>
