...
export function BrowseTrainingRightSidebar() {
  ...
  const fetchData = async () => {
    try {
      const [coursesRes, pendingRes] = await Promise.all([
        ...
        fetch("/api/student/training/purchases/pending")
      ]);
      if (coursesRes.ok) {
      ...
      if (pendingRes.ok) {
        ...
        if (Array.isArray(pData)) setPendingPurchases(pData);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener("trainingPurchaseAdded", handleUpdate);
    return () => window.removeEventListener("trainingPurchaseAdded", handleUpdate);
  }, []);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, purchaseId: string) => {
    ...
    setTimeout(async () => {
      try {
        const res = await fetch("/api/student/training/receipt", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            purchaseId,
            receiptUrl: "https://example.com/fake-receipt.jpg"
          })
        });
  ...
  const handleCancelPurchase = async (purchaseId: string) => {
    ...
    try {
      const res = await fetch(`/api/student/training/buy?purchaseId=${purchaseId}`, {
        method: "DELETE"
      });
